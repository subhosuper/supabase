...
You can have a client send a message and then receive it by setting Broadcast's `self` config to `true`.
...
If the `ack` config is not set to `true` when creating the channel, the promise returned by call to `channelD.send` will resolve immediately.
